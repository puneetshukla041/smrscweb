...
// This stops them from blocking the initial page load.
const Overview = dynamic(() => import('../../components/explore/Overview'), {
  ssr: false, // Prevents server-side rendering blocking
            ...
            <motion.div key={index} variants={itemVariants} className="group relative cursor-pointer" onClick={() => toggleFAQ(index)}>
              ...
              <div className="relative z-10 px-6 py-6 md:px-8 pointer-events-none">
                <div className="flex items-center justify-between w-full text-left">
                  <span className={`font-manrope text-lg font-medium tracking-tight transition-colors duration-300 pr-8 ${isOpen ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
                    {faq.question}
                  </span>
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={springTransition} className="overflow-hidden">
                      <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 0.7 }} exit={{ y: 20, opacity: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="pt-4 pb-2 text-base leading-relaxed text-gray-300 max-w-3xl font-manrope">
                        {faq.answer}
                      </motion.p>
                    </motion.div>
                  )}
...
const ExploreContent = () => {
  ...
  useEffect(() => {
    // Double requestAnimationFrame ensures the browser completes the visual paint
    // of the Hero image before React starts doing heavy background work.
  ...
  return (
    <main className="flex-grow w-full flex flex-col items-center overflow-x-hidden">
        ...
        <section className="w-full bg-transparent flex justify-center">
          ...
          >
            <div style={{ marginTop: "98px", marginBottom: "65px", marginLeft: "114px", marginRight: "114px", width: "100%", maxWidth: "1723px", aspectRatio: "1723 / 876" }} className="relative">
              <Image
                src="/images/explore/hero.webp" 
                alt="Explore Hero"
                fill
                priority // Priority ensures this loads before anything else
                fetchPriority="high" // Force browser to prioritize download
        ...
        {/* This entire block stays hidden until the hero image is successfully painted */}
        {loadHeavyContent && (
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="flex flex-col items-center w-full max-w-[1440px] px-4 md:px-0"
          >
